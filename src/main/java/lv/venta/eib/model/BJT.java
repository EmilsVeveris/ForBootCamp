...
public class BJT extends Transistor {
	...
	@Size(min = 0, max = 5)
	@Column(name="BjtType")
	private String bjtType;
	
	@Min(0)
	@Column(name="HfeGain")
	private int hfeGain;

	public BJT(@Size(min = 0, max = 15) String componentId, @Size(min = 0, max = 80) String description,
			...
			@Min(0) int maxVoltage, @Min(0) float maxCurrent, @NonNull @Size(min = 0, max = 5) String bjtType,
			@Min(0) int hfeGain) {
		super(componentId, description, datasheet, tolerance, casetype, casestyle, room, count, maxVoltage, maxCurrent);
		this.bjtType = bjtType;
		this.hfeGain = hfeGain;
	}
